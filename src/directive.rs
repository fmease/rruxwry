...
impl<'src> InstantiatedDirectives<'src> {
    fn adjoin(&mut self, directive: BareDirective<'src>) {
        match directive {
            BareDirective::AuxBuild { path: (path, span) } => {
                self.dependencies.push(ExternCrate::Unnamed { path: (path, Some(span)) });
            }
            BareDirective::AuxCrate { name, path } => {
                self.dependencies.push(ExternCrate::Named {
                    name,
                    path: path.map(|(path, span)| (path.into(), Some(span))),
                });
            }
    ...
    fn parse_base_directive(
        ...
        Ok(Some(match source {
            "aux-build" => {
                ...
                BareDirective::AuxBuild { path: self.take_remaining_line() }
            }
    ...
    fn emit(self, file: SourceFileIndex, cx: Context<'_>) {
        ...
        match self {
            ...
            Self::UndeclaredRevision { revision: (revision, span), available } => {
                ...
                let it = error(fmt!("undeclared revision `{revision}`")).highlight(span, cx);
