...
impl<'src> InstantiatedDirectives<'src> {
    fn adjoin(&mut self, directive: SimpleDirective<'src>) {
        match directive {
            ...
            SimpleDirective::Flags(flags) => self.verbatim.arguments.extend(flags),
            ...
            SimpleDirective::RustcEnv { key, value } => {
                self.verbatim.variables.push((key, Some(value)));
            }
            SimpleDirective::UnsetRustcEnv(key) => {
                self.verbatim.variables.push((key, None));
            }
...
impl<'src> Parser<'src> {
    ...
    fn parse_base_directive(
        ...
        Ok(Some(match source.bare {
            ...
            "compile-flags" => return self.parse_flags().map(Some),
            "doc-flags" => {
                match self.scope {
                    Scope::HtmlDocCk | Scope::JsonDocCk => {}
                    Scope::Base => {
            ...
            //        `run-flags`, `exec-env`, `unset-exec-env`,
            //        `proc-macro`, `aux-bin`,
    ...
    fn parse_flags(&mut self) -> Result<SimpleDirective<'src>, Error<'src>> {
        ...
        Ok(SimpleDirective::Flags(arguments))
    }
