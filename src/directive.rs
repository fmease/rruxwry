...
impl<'src> Parser<'src> {
    ...
    fn parse_base_directive(
        ...
        Ok(Some(match source.bare {
            "aux-build" => {
                ...
                SimpleDirective::AuxBuild { path: self.take_remaining_line() }
            }
            ...
            "aux-crate" => {
                ...
                let name = self.take_while(|char| char != '=' && !char.is_whitespace())?.bare;
            ...
            "compile-flags" => {
                ...
                let arguments = self.take_remaining_line().bare.split_whitespace().collect();
            ...
            "edition" => {
                ...
                // We're doing this two-step process — (greedy) lexing followed by validation —
                // to be able to provide a better error message.
                let edition = self.take_while(|char| !char.is_whitespace())?.bare;
            ...
            "rustc-env" => {
                ...
                let line = self.take_remaining_line().bare;
            ...
            "unset-rustc-env" => {
                self.parse_separator(Padding::No)?;
                let variable = self.take_remaining_line().bare;
    ...
    fn expect(&mut self, expected: char) -> Result<(), Error<'src>> {
        ...
        if char != expected {
            let span = self.span(LocalSpan::with_len(index, len_utf8(char)));
    ...
    fn take_while(
        ...
        while let Some((index, char)) = self.peek() {
            ...
            end = Some(index + len_utf8(char));
            self.advance();
        }

        if let Some((start, end)) = start.zip(end) {
            Ok(self.spanned(LocalSpan::new(start, end)))
        } else {
            Err(Error::UnexpectedEndOfInput)
        }
    }

    fn take_remaining_line(&mut self) -> Spanned<&'src str> {
        // FIXME: Should we instead bail on empty lines?
        self.take_while(|char| char != '\n')
            // FIXME: Don't create a sham span
            .unwrap_or_else(|_| Spanned::sham(""))
    }
