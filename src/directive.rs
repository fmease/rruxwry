...
use crate::{
    builder::QueryMode,
    command::{CrateNameRef, Edition, ExternCrate},
    parser,
    utility::{default, Tag},
};
...
impl<'src> DirectivesParser<'src> {
    fn new(source: &'src str, query: Option<QueryMode>) -> Self {
        Self {
            parser: parser::SourceFileParser::new(source),
            query,
            directives: default(),
        }
    }
        ...
        while let Some(token) = self.parser.peek() {
                ...
                && let Some(directive) = comment.strip_prefix("//@")
            {
                match DirectiveParser::new(directive).execute() {
                    ...
                    Err(Error {
                        kind: ErrorKind::UnknownDirective(directive),
                        ..
...
pub(crate) struct InstantiatedDirectives<'src> {
    ...
    pub(crate) compile_flags: Vec<&'src str>,
    pub(crate) edition: Option<Edition>,
    pub(crate) force_host: bool,
    pub(crate) no_prefer_dynamic: bool,
    pub(crate) revisions: FxHashSet<&'src str>,
    pub(crate) rustc_env: Vec<(&'src str, &'src str)>,
    pub(crate) unset_rustc_env: Vec<&'src str>,
}
...
impl<'src> InstantiatedDirectives<'src> {
    fn adjoin(&mut self, directive: DirectiveKind<'src>) {
        match directive {
            ...
            DirectiveKind::CompileFlags(flags) => self.compile_flags.extend(flags),
            ...
            DirectiveKind::RustcEnv { key, value } => self.rustc_env.push((key, value)),
            DirectiveKind::UnsetRustcEnv(var) => self.unset_rustc_env.push(var),
        }
    }
}
        ...
        let kind = match directive {
            "aux-build" => {
                self.parse_separator(Padding::Yes)
                    .map_err(|error| error.context(context))?; // FIXME: audit AllowPadding
            ...
            "aux-crate" => {
                ...
                let path = self
                    .consume(|char| char == '=')
                    .then(|| self.take_remaining_line());
                DirectiveKind::AuxCrate { name, path }
            }
            "build-aux-docs" => DirectiveKind::BuildAuxDocs,
            "compile-flags" => {
                self.parse_separator(Padding::Yes)
                    .map_err(|error| error.context(context))?; // FIXME: audit AllowPadding (before)
