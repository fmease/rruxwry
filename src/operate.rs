...
fn build_directive_driven<'a>(
    ...
    let krate = Crate {
        ...
        edition,
        name: krate.name,
        typ: directives.prefer_dylib.apply(krate.typ),
    };
    ...
    match test {
        Test::Yes(bless) => {
            let output = cmd.execute_capturing_output(cx)?;
            compare(path, output, bless)?;
        }
        Test::No => cmd.execute(cx)?.exit_ok().map_err(io::Error::other)?,
    }
...
fn compare(path: &Path, output: std::process::Output, bless: Bless) -> io::Result<()> {
