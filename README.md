...
<!-- FIXME: We now have a `build` vs `doc`. README is super out of date! -->
...
2. Executing `rruxwry d file.rs -X` (or `rruxwry d file.rs -Xo`).
...
Compared to a raw `rustdoc file.rs` invocation, a `rruxwry d file.rs` invocation has some goodies in store.
...
We can easily trigger this code path by creating a dependent crate containing `pub use krate::*;` re-exporting the crate `krate` we're actually interested in. `rruxwry d -X` does this step for us. It generates a dummy crate called `u_⟨name⟩` and invokes `rustc` & `rustdoc` for us.
...
As one would expect, if the first argument begins with a `+`, it will be interpreted as a *[rustup]* toolchain name. Examples: `rruxwry +nightly {b,d} file.rs`, `rruxwry +stage2 {b,d} file.rs`.
